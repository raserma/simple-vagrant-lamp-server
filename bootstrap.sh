...
# Flush privileges
sudo mysql -u root -p'1234' -e 'FLUSH PRIVILEGES';

# Create new user guest on demo db
sudo mysql -u root -p'1234' -e "USE mysql; create database demo; GRANT ALL PRIVILEGES ON demo.* TO 'guest'@'localhost' IDENTIFIED BY 'guest' WITH GRANT OPTION; UPDATE user SET Password=PASSWORD('guest') WHERE user='guest';" 
